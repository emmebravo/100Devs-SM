...
module.exports = {
  ...
  createPost: async (request, response) => {
    try{
        await Post.create({userId: request.user.id, title: request.body.title, caption: request.body.caption, user: request.user.userName})
  ...
  likePost: async (request, response) => {
    try{
      let post = await Post.findOneAndUpdate({_id: request.params.id}, {$inc: {likes: 1}})
      response.render("post.ejs", { post: post, user: request.user })
    }catch(error){
      console.error(error)
    }
  ...
  getProfile: async (request, response) => {
    try {
      const myPosts = await Post.find({userId: request.user.id})
